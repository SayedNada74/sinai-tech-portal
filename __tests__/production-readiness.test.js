...
async function runTests() {
    ...
    assert(tooManyRequests > 50 && successRequests <= 20, `Rate Limiter effectively stopped barrage (Success: ${successRequests}, Blocked: ${tooManyRequests})`, "Rate limiter did not block enough requests.");
    ...
    let cloudWrites = 0;
    async function mockCloudUpdate(snapshot) {
        cloudWrites++;
        await new Promise(resolve => setTimeout(resolve, 50)); // Mock 50ms latency
    }
    async function queueUpdate(newGpa, isFlush = false) {
        sharedState.gpa = newGpa;
        if (!isFlush)
            sharedState.clicks++; // Only count external clicks
        if (isSyncing) {
            hasPending = true;
            return;
        }
    ...
    assert(sharedState.gpa === 250 && sharedState.clicks === 250, "Race condition test passed: No lost updates in local state.", `State corrupted. GPA: ${sharedState.gpa}, Clicks: ${sharedState.clicks}`);
    assert(cloudWrites < 250 && cloudWrites > 0, `Coalescing worked: 250 clicks resulted in only ${cloudWrites} network writes.`, `Too many or zero writes: ${cloudWrites}`);
    ...
    if (!supabaseUrl || !anonKey) {
        ...
        assert(isBlocked, "RLS / IDOR: Unauthenticated PATCH to profiles is rejected (RLS filtered)", `Expected rejection/empty, got ${maliciousPatch.status} with body: ${maliciousPatchText}`);
        ...
        assert(fakeJwtReq.status === 401, "Authorization Bypass: Fake JWT rejected by admin endpoint", `Expected 401, got ${fakeJwtReq.status}`);
        // 3. Security Headers Verification
        const rootReq = await fetch('http://localhost:3001/');
        const csp = rootReq.headers.get('content-security-policy');
        assert(csp !== null && csp.includes('frame-src') && csp.includes('self'), "Security Headers: Content-Security-Policy is active and restrictive", "CSP is missing or malformed");
    }
    console.log("\n=====================================================");
