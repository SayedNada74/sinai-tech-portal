...
interface CourseDetailInteractiveProps {
  course: Course;
  relatedCourses: Course[];
}

export function CourseDetailInteractive({ course, relatedCourses }: CourseDetailInteractiveProps) {
  ...
  React.useEffect(() => {
    if (course) {
      addRecentlyViewed(course.code, "course", course.arabic, `/courses/${getCourseSlug(course)}`);
    }
  }, [course]);
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          onClick={() => {
            ...
            toggleBookmark(course.code, "course", course.arabic, `/courses/${getCourseSlug(course)}`);
          }}
        ...
        >
          {bookmarked ? (
            ...
            <Bookmark className="h-4 w-4 text-primary dark:text-sky-400 stroke-[2.4]" />
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="border border-zinc-200/50 dark:border-zinc-800/50 bg-white dark:bg-zinc-900 shadow-sm">
            <CardHeader className="pb-2">
              <CardTitle className="text-base font-bold flex items-center gap-2 text-zinc-900 dark:text-zinc-50">
                <BookOpen className="h-4.5 w-4.5 text-primary dark:text-zinc-500" />
          ...
          <Card className="border border-zinc-200/50 dark:border-zinc-800/50 bg-white dark:bg-zinc-900 shadow-sm">
            <CardHeader className="pb-2">
              <CardTitle className="text-base font-bold flex items-center gap-2 text-zinc-900 dark:text-zinc-50">
                <GraduationCap className="h-4.5 w-4.5 text-primary dark:text-zinc-500" />
            ...
            <CardContent className="space-y-3">
              {courseResources.length > 0 ? (
                ...
                <div className="text-center py-6 text-xs text-zinc-400 dark:text-zinc-500">
                  {t("لا توجد مصادر دراسية مضافة لهذه المادة بعد. يمكنك إضافة مصادر من صفحة المصادر.", "No study resources added for this course yet."
                  )}
                </div>
              )}
            </CardContent>
          ...
          <Card className="border border-zinc-200/50 dark:border-zinc-800/50 bg-white dark:bg-zinc-900 shadow-sm">
            <CardHeader className="flex flex-row justify-between items-center pb-2">
              <div>
                <CardTitle className="text-base font-bold flex items-center gap-2 text-zinc-900 dark:text-zinc-50">
                  <MessageSquare className="h-4.5 w-4.5 text-primary dark:text-zinc-500" />
              ...
              {course.prerequisites.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {course.prerequisites.map((pre) => (
                    <Link href={`/courses/${getCourseSlug({ code: pre })}`} key={pre}>
          ...
          <Card className="border border-zinc-200/50 dark:border-zinc-800/50 bg-white dark:bg-zinc-900 shadow-sm">
            ...
            <CardContent className="pt-4 space-y-3.5">
              {relatedCourses.map((rc) => (
                <Link href={`/courses/${getCourseSlug(rc)}`} key={rc.code} className="block group">
          ...
          <Card className="border border-zinc-200/50 dark:border-zinc-800/50 bg-white dark:bg-zinc-900 shadow-sm">
            <CardHeader className="pb-3 border-b border-zinc-100 dark:border-zinc-800/60">
              <CardTitle className="text-sm font-bold flex items-center gap-2 text-zinc-900 dark:text-zinc-50">
                <Users className="h-4.5 w-4.5 text-primary dark:text-zinc-500" />
            ...
            <CardContent className="pt-4 space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5">
                  <Star className={`h-4.5 w-4.5 ${avgRating > 0 ? "fill-amber-400 text-amber-400" : "text-zinc-300 dark:text-zinc-600"}`} />
