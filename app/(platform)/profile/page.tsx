...
export default function ProfilePage() {
      ...
      if (typeof window !=="undefined") {
        try {
          ...
          if (draftStr) {
            ...
            // Do not load avatar from draft to prevent stale base64 strings from overriding fresh DB data
  ...
  // Persist draft on changes only if dirty
  React.useEffect(() => {
    if (user && isInitialized.current && typeof window !=="undefined") {
      const draft = {
        ...
        portfolioUrl,
        skills,
        // Exclude avatar from draft to prevent localStorage quota issues with large base64 strings
        cvUrl,
        projects,
        publicSkills,
        publicProjects
      ...
      // Only save draft if there are actual changes compared to the user object
      const hasChanges = 
        nameAr !== (user.nameAr || "") ||
        nameEn !== (user.nameEn || "") ||
        bio !== (user.bio || "") ||
      ...
      if (hasChanges) {
        localStorage.setItem(`su_profile_draft_${user.id}`, JSON.stringify(draft));
      } else {
        localStorage.removeItem(`su_profile_draft_${user.id}`);
      }
    }
  }, [user, nameAr, nameEn, email, level, studentId, bio, github, linkedin, portfolioUrl, skills, cvUrl, projects, publicSkills, publicProjects]);
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-6 rounded-3xl bg-gradient-to-r from-cyan-600/10 via-teal-600/10 to-transparent border border-cyan-500/30">
          <div>
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
              {t("إدارة بيانات الحساب الإداري، متابعة مستوى الصلاحيات الأكاديمية، وضبط إعدادات الأمان.","Manage admin credentials, inspect system clearances, and configure safety settings."
              )}
            </p>
          </div>
        </div>
            ...
            <Card className="border border-zinc-200/60 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm">
              ...
              <CardContent className="pt-6 space-y-5">
                ...
                <div className="pt-4 border-t border-zinc-100 dark:border-zinc-800 flex justify-end">
                  <Button type="submit" disabled={isSaving} className="bg-cyan-600 hover:bg-cyan-700 text-white font-bold text-xs px-6 h-11 rounded-xl cursor-pointer">
                    {isSaving ? t("جاري الحفظ...","Saving...") : t("حفظ التغيرات الإدارية","Save Admin Profile")}
                  </Button>
                </div>
              </CardContent>
            </Card>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6">
        <div>
          ...
          <p className="text-xs sm:text-sm text-zinc-550 dark:text-zinc-400 mt-1">
            {t("قم بإعداد بوابتك المهنية، رفع صورتك الشخصية، واستعراض شاراتك الأكاديمية.","Setup your professional portal, upload your profile picture, and view academic badges."
            )}
          </p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6 lg:col-span-1">
            <Card className="border border-zinc-200/50 dark:border-zinc-800/50 bg-white dark:bg-zinc-900 shadow-sm text-center">
              <CardContent className="pt-8">
                ...
                <h3 className="font-extrabold text-base text-zinc-950 dark:text-zinc-50">
                  {getLocalizedUserName({ nameAr, nameEn, name: user?.name }, lang) || t("طالب مستجد", "Freshman Student")}
                </h3>
            ...
            {(cvUrl?.trim() || portfolioUrl?.trim() || linkedin?.trim() || github?.trim()) && (
                ...
                <CardContent className="pt-4 space-y-2.5">
                  ...
                  {portfolioUrl && portfolioUrl.trim().length > 0 && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center justify-between p-2.5 rounded-xl bg-cyan-500/10 border border-cyan-500/20 text-cyan-700 dark:text-cyan-400 hover:bg-cyan-500/20 transition-all font-bold text-xs cursor-pointer"
                    >
                      <span className="flex items-center gap-2">
                  ...
                  {linkedin && linkedin.trim().length > 0 && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center justify-between p-2.5 rounded-xl bg-blue-500/10 border border-blue-500/20 text-blue-700 dark:text-blue-400 hover:bg-blue-500/20 transition-all font-bold text-xs cursor-pointer"
                    >
                      <span className="flex items-center gap-2">
                  ...
                  {github && github.trim().length > 0 && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center justify-between p-2.5 rounded-xl bg-zinc-100 dark:bg-zinc-800/60 border border-zinc-200 dark:border-zinc-700 text-zinc-900 dark:text-zinc-100 hover:bg-zinc-200 transition-all font-bold text-xs cursor-pointer"
                    >
                      <span className="flex items-center gap-2">
                        <Globe className="h-4 w-4" />
        ...
        <div className="space-y-6">
            ...
            <CardHeader className="bg-zinc-50/50 dark:bg-zinc-950/20 border-b border-zinc-100 dark:border-zinc-850 p-4 sm:p-5">
              <div className="flex items-center gap-3">
                <div className="h-9 w-9 rounded-2xl bg-sky-100 dark:bg-sky-500/20 flex items-center justify-center shrink-0 shadow-inner">
                  <ShieldAlert className="h-5 w-5 text-primary dark:text-sky-400" />
                </div>
                <div>
          ...
          <div className="flex justify-center sm:justify-end gap-3.5 pt-4">
            <Button type="submit" className="w-full sm:w-auto px-10 shadow-md font-bold" isLoading={isSaving} disabled={isSaving}>
        ...
        {isAddProjOpen && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div
              ...
              className={`bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-850 rounded-3xl w-full max-w-md max-h-[90vh] flex flex-col shadow-2xl overflow-hidden ${isRtl ? "text-right" : "text-left"}`}
              dir={dir}
            >
              ...
              <form onSubmit={handleAddProject} className="p-5 sm:p-6 space-y-4 overflow-y-auto flex-1">
