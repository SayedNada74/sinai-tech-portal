...
export default function RoadmapsPage() {
  ...
  const [mode, setMode] = React.useState<"global" | "faculty">("global");
  const [selectedGlobalTrackId, setSelectedGlobalTrackId] = React.useState<string>("frontend");
  const [activeFacultyRoadmapId, setActiveFacultyRoadmapId] = React.useState<string>("frontend");
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-zinc-900 dark:text-zinc-50 flex items-center gap-2.5">
            <Compass className="h-7 w-7 text-cyan-500" />
            {t("مسارات وخارطة طريق التعلّم المهنية", "Professional Career Roadmaps")}
          </h1>
          <p className="text-xs sm:text-sm text-zinc-500 dark:text-zinc-400 mt-1">
            {t(
              "دليل شامل يجمع بين خرائط التكنولوجيا العالمية والتوجيهات الأكاديمية الخاصة بالكلية.",
              "Comprehensive roadmaps combining global tech standards with faculty guidance."
            )}
          </p>
        </div>
      </div>
